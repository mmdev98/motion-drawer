...
type PropsForFeature<
  ...
  TProps
> = TPassedInFeatures extends TForFeature ? TProps : object;
...
export function useRender() {
  const mergeRefs = useMergeRefsFn();
  ...
  if (Component === Fragment) {
    if (
      ...
      if (
        ...
        const childProps = resolvedChildren.props as {
          className: string | (() => string);
        } | null;

        const childPropsClassName = childProps?.className;
        const newClassName =
        ...
        const mergedProps = mergePropsAdvanced(
