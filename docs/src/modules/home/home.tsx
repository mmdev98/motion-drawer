...
export function Home() {
  ...
  const comments = [
    ...
    },
    {
      ...
      author: "Lisa Park",
      content:
        "The customization options are exactly what I needed. Thanks! ✨",
  ...
  return (
    <div className="min-h-dvh flex flex-col p-4 justify-center bg-bg items-center">
      <div className="space-y-6 sm:space-y-8 text-center">
        <h1 className="text-3xl font-bold tracking-tight sm:text-5xl lg:text-6xl">
          Motion Drawer
        </h1>
        <p className="mx-auto text-base max-w-[600px] leading-relaxed text-neutral-600 dark:text-neutral-400">
          Motion Drawer is a headless drawer with smooth animations built on top
          of the{" "}
          <Link href="https://motion.dev" className="text-info" target="_blank">
            Motion
          </Link>{" "}
          library and easily integrates with any headless libraries such as{" "}
          <Link
      ...
      <div className="mt-6 flex">
        <Button as={Link} href="/getting-started" variant="plain" color="info">
          Documentation
        </Button>
        <Button
          variant="outlined"
        ...
        {open && (
          <Dialog open onClose={setOpen} className="fixed z-50">
            <DialogPanel
              as={Drawer}
              className={cn(
                "z-2 w-full sm:w-[calc(100%-2rem)] border lg:max-w-96 bg-white dark:bg-[#1F1F1FFF]",
              )}
              defaultOpen={open}
